...
public class ReceiverFeignService implements ReceiverRestService {
    ...
    public List<String> getAddresses(List<String> codes, String user){
        return client.getReceiversFromCodes(codes, user).stream().map(ReceiverDTO::getAddress).toList();
    }

    public List<String> addressesFallback(List<String> codes, Throwable t){
        throw new RuntimeException("Unable to get receiver addresses");
    }
}
